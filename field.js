/* Live particle field. Signal source: pointer by default, webcam motion when enabled. */
(() => {
...
const state = { px: .5, py: .45, tx: .5, ty: .45, energy: 0, tEnergy: 0, cam: false, scroll: 0 };

/* ---------- HUD + pointer ---------- */
addEventListener('pointermove', e => {
  if (state.cam) return;
  state.tx = e.clientX / innerWidth; state.ty = e.clientY / innerHeight;
  state.tEnergy = Math.min(1, state.tEnergy + .06);
}, { passive: true });
addEventListener('scroll', () => { state.scroll = scrollY / Math.max(1, document.body.scrollHeight - innerHeight); }, { passive: true });

/* ---------- webcam motion ---------- */
let vid, mctx, prev = null, mw = 64, mh = 48, monctx;
...
async function enableCam() {
  try {
    ...
    hud.btn.onclick = () => { stream.getTracks().forEach(t => t.stop()); state.cam = false; prev = null; hud.btn.textContent = 'enable camera'; hud.btn.onclick = enableCam; if (monctx) monctx.clearRect(0, 0, mw, mh); };
  } catch (err) {
...
const vs = `attribute vec2 a_uv;uniform float u_t;uniform vec2 u_p;uniform float u_e;uniform float u_sc;uniform float u_asp;varying float v_i;
...
const COLS = 190, ROWS = 118, pts = new Float32Array(COLS * ROWS * 2);
let k = 0;
...
const u_t = U('u_t'), u_p = U('u_p'), u_e = U('u_e'), u_sc = U('u_sc'), u_asp = U('u_asp');
...
function frame(now) {
  ...
  readMotion();
  state.px += (state.tx - state.px) * .07;
  state.py += (state.ty - state.py) * .07;
  state.energy += (state.tEnergy - state.energy) * .08;
  if (!state.cam) state.tEnergy *= .965;
  ...
  if (hud.src) {
    hud.src.textContent = state.cam ? 'camera' : 'pointer';
